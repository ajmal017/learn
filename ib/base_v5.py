...
async def margins(ib: IB, c, **kwargs) -> pd.DataFrame:
    ...
    lot = [o.totalQuantity if ct.exchange == 'NSE' else 100 for ct, o in c]
    ...
    df_mgns = df_mgns.assign(margin=np.where(df_mgns.margin > 1e7, np.nan, df_mgns.margin),
                             comm=np.where(df_mgns.comm > 1e7, np.nan, df_mgns.comm))
...
def prepOpts(BLK_SIZE: int = 100):
    ...
    df_symlots = pd.read_pickle('./data/df_' + MARKET.lower() + '_symlots.pkl')
...
if __name__ == "__main__":
    ...
    with IB().connect(HOST, PORT, CID) as ib:
        ...
        # qualified options
        df_raw_opts = ib.run(execute_seq_task_pool(ib=ib,
                                                   cts=ct_blks,
                                                   MARKET=MARKET,
                                                   algo=optscs,
                                                   CONCURRENTS=30,
        ...
        df_raw_opts = pd.read_pickle(
            './data/df_' + MARKET.lower() + '_qopts.pkl')
    ...
    # Integrate all and pickle

    df_unds = pd.read_pickle(
        './data/df_' + MARKET.lower() + '_unds.pkl')

    df_margins = pd.read_pickle(
        './data/df_' + MARKET.lower() + '_opt_margins.pkl')

    df_prices = pd.read_pickle(
        './data/df_' + MARKET.lower() + '_opt_prices.pkl')

    # ... replace missing undPrice with price from df_und
    old_price = df_prices[df_prices.undPrice.isnull()].symbol.map(
        df_unds.set_index('symbol').undPrice.to_dict())
    df_prices.loc[df_prices.undPrice.isnull(), 'undPrice'] = old_price

    # ... merge price, margins and lots
    cols_to_use = list(df_margins.columns.difference(
        df_prices.columns)) + ['conId']
    df = df_prices.set_index('conId').join(
        df_margins[cols_to_use].set_index('conId')).reset_index().drop('secType', 1)
    ...
    # ... pickle
    df.to_pickle(FSPATH + 'df_' + MARKET.lower() + 'opts.pkl')
